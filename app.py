...
def process_info_row(thresh_img, anchor, offset, gap, box_s):
    ...
    for i in range(10):
        x = x_start + (i * gap)
        # 這裡原本 Colab 是 y:y+box_s, x:x+box_s (Numpy 切片順序是 y, x)
...
def analyze_paper_stream_lite(image):
    # 1. 轉灰階 (注意：PIL 轉 Numpy 預設是 RGB，OpenCV 習慣 BGR，但轉灰階差異不大，維持原本邏輯)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    
    # 2. 自適應二值化 (您 Colab 設定: 35, 1)
